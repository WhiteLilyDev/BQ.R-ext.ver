...
static unsigned int SSD1306_write8(unsigned char reg, unsigned char val) {
...
static unsigned int SSD1306_write_cmd(unsigned char cmd) {
    return SSD1306_write8(0x00, cmd);
}

static unsigned int SSD1306_write_data(unsigned char data) {
    return SSD1306_write8(0x40, data);
}
...
void OLED_Fill_Row(unsigned char fill_Data, unsigned char row) {
    if (row > 8) return;
    SSD1306_write_cmd(0xb0 + row);        //page0-page1
    SSD1306_write_cmd(0x00);        //low column start address
    SSD1306_write_cmd(0x10);        //high column start address
    for (int n = 0; n < 128; n++) {
        SSD1306_write_data(fill_Data);
