 ...
 *      TYPEDEFS
 **********************/
lv_disp_draw_buf_t * disp_drv_p;		//屏幕驱动结构体地址
/**********************
 *  STATIC PROTOTYPES
 **********************/
static void disp_init(void);

static void disp_flush(lv_disp_t * disp_drv, const lv_area_t * area, const lv_color_t * color_p);
#if LV_USE_GPU
...
void lv_port_disp_init(void)
{
    ...
    lv_disp_draw_buf_init(&draw_buf_dsc_1, draw_buf_1, NULL, LV_HOR_RES_MAX * 10);   /*Initialize the display buffer*/
...
static void disp_flush(lv_disp_t * disp_drv, const lv_area_t * area, const lv_color_t * color_p)
{
    ...
    #if LCD_USE_GC9A01
        Gc9a01_LCD_Color_Fill(area->x1,area->y1,area->x2,area->y2,(uint16_t*)color_p);
    #endif
    #if LCD_USE_ST7789
        ST7789_LCD_Color_Fill(area->x1,area->y1,area->x2,area->y2,(uint16_t*)color_p);
    #endif
