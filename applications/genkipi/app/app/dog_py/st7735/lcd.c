...
{	
    ...
    //AL_SPI_Transmit(&hspi1,&dat,1, 0xffff);

	uint32_t ret = hi_spi_host_write(HI_SPI_ID_0, &dat, 1);
...
void LCD_ShowChar96(u16 x,u16 y,u8 num,u8 mode,u16 color)
{
	...
	// {
		...
		{ 
			//temp=asc2_4896[(u16)num*96+pos];		 //调用1608字体
			for(int i=0; i < 6; i++){
				temp=asc2_4896[(u16)num*576+pos*6+i];	
				for(t=0;t<8;t++)
		    	{                 
		        	if(temp&0x01){
						LCD_WR_DATA(color);
...
void LCD_ShowGif(u16 x1,u16 y1,u16 x2,u16 y2,const u16* picture,u32 len)
{
	  ...
	  { 	
		OLED_DC_Set();//写数据
		LCD_WR_DATA8(picture[i]>>8);
		LCD_WR_DATA8(picture[i]);  
		//   OLED_DC_Set();//写数据
		// 	LCD_Writ_Bus(picture[i*2+1]);
		// 	LCD_Writ_Bus(picture[i*2]);

		//   //LCD_WR_DATA(picture[i]);	
		    // LCD_WR_DATA8(picture[i*2+1]);		
			// LCD_WR_DATA8(picture[i*2]);	 
...
void LCD_ShowChar42(u16 x,u16 y,u8 num,u8 mode,u16 color)
{
	...
	printf("num=%d\n",num);
	LCD_Address_Set(x,y,x+24-1,y+42-1);      //设置光标位置 

	for(pos=0;pos<42;pos++)
	{ 
		//temp=asc2_4896[(u16)num*96+pos];		 //调用1608字体
		for(int i=0; i < 3; i++){
			temp=asc_4842[(u16)num*126+pos*3+i];	
			for(t=0;t<8;t++)
			{                 
				if(temp&0x01){
					LCD_WR_DATA(color);
...
{  
    ...
    LCD_Address_Set(x,y,x2,y2); //设置一个汉字的区域
    //size1=size*size/8;//一个汉字所占的字节
	temp+=index*size1;//写入的起始位置
	for(j=0;j<size1;j++)
	{
